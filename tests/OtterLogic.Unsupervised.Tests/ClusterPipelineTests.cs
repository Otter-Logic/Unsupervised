...
public sealed class ClusterPipelineTests
{
    ...
    public ClusterPipelineTests(ITestOutputHelper output) => _output = output;

    private static ClusterPipelineOptions Matching(int groups) => new()
    {
        Groups = groups,
        LogTransform = true,
        NormaliseRows = false,
    ...
    public void ReachesTheSameQualityOfFitAsScikitLearn()
    {
        ...
        var result = ClusterPipeline.Group(raw, Matching(fixture.Int("components")));
    ...
    public void OrdersGroupsByShareSoLabelsAreStable()
    {
        ...
        var result = ClusterPipeline.Group(raw, Matching(4));
    ...
    public void DropsColumnsWithNoVariance()
    {
        ...
        var result = ClusterPipeline.Group(planar, Matching(3));
    ...
    public void WeightsChangeTheGrouping()
    {
        ...
        var even = ClusterPipeline.Group(raw, Matching(4));
        var momentLed = ClusterPipeline.Group(raw, Matching(4) with
        {
            Weights = new[] { 0.25, 0.25, 0.25, 3.0, 3.0, 3.0 },
        });
        var forcesOnly = ClusterPipeline.Group(raw, Matching(4) with
        {
    ...
    public void RowNormalisationGroupsByShapeRatherThanSize()
    {
        ...
        var bySize = ClusterPipeline.Group(raw, Matching(4));
        var byShape = ClusterPipeline.Group(raw, Matching(4) with
        {
            NormaliseRows = true,
            LogTransform = false,
        });
    ...
    public void SweepsGroupCountAndTracksScikitLearn()
    {
        ...
        var ours = ClusterPipeline.ChooseGroupCount(
